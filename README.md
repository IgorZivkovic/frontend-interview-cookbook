...
This section explores advanced React concepts beyond basic hooks and components. As of July 2026, React 19.2.x is the current stable line; npm's latest stable patch is 19.2.7. Applications using React Server Components or Server Functions should keep React and their RSC integration packages coordinated on the latest patched 19.2.x release. [CVE-2026-23870](https://github.com/react/react/security/advisories/GHSA-rv78-f8rc-xrxh) affected `react-server-dom-webpack`, `react-server-dom-parcel`, and `react-server-dom-turbopack` versions 19.2.0 through 19.2.5, so 19.2.6 is the minimum fixed 19.2.x version after the May 2026 advisory; 19.2.7 is the current recommendation.
...
- **What security context matters for React Server Components?** RSC and Server Functions depend on framework/bundler integration. For affected `react-server-dom-*` packages, 19.2.6 is the minimum fixed 19.2.x version after the May 2026 [CVE-2026-23870 advisory](https://github.com/react/react/security/advisories/GHSA-rv78-f8rc-xrxh); applications should install the latest coordinated React/RSC packages—19.2.7 as of July 2026. Client-only React applications and projects without an RSC-capable framework, bundler, or plugin are not affected by this RSC vulnerability.
